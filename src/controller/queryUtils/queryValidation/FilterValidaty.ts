...
export const ROOM_KEYS_VALUES = new Map<string, string>([
	["fullname", "string"],
	["shortname", "string"],
	["number", "string"],
	["name", "string"],
...
export const COURSES_STRING_KEYS: string[] = ["dept", "id", "instructor", "title", "uuid"];
export const COURSES_NUM_KEYS: string[] = ["avg", "pass", "fail", "audit", "year"];
export const ROOMS_NUM_KEYS: string[] = ["lat", "lon", "seats"];
export const ROOMS_STRING_KEYS: string[] = ["fullname", "shortname", "number", "name", "address",
	"type", "furniture", "href"
];
...
function mComparator(filterObject: any, dataSetsReferencedSoFar: Set<string>) {
	...
	if (!COURSES_NUM_KEYS.includes(key.split("_")[1])
		&& !ROOMS_NUM_KEYS.includes(key.split("_")[1])
	) {
		return false;
	}
	...
	if (typeof value !== QUERY_KEYS_VALUES.get(dataset[1]) && typeof value !== ROOM_KEYS_VALUES.get(dataset[1]) ) {
		return false;
	}
...
function isFilter(filterObject: any, dataSetsReferencedSoFar: Set<string>) {
	...
	if (key === undefined
		|| value === undefined
		|| (!COURSES_STRING_KEYS.includes(key.split("_")[1])
		&& !ROOMS_STRING_KEYS.includes(key.split("_")[1]))
	) {
		return false;
	}
